...
{
    RenderDevice::RenderDevice(const Descriptor& descriptor, const bgfx::ViewId viewID) : ViewID_ { viewID }
    {
        ...
        this->Width_ = descriptor.Display.Width;
        this->Height_ = descriptor.Display.Height;
    }

    template <const std::size_t Count>
    static auto LoadShaders(const std::span<const std::string_view, Count> names, const std::span<bgfx::ShaderHandle, Count> out, const std::string_view suffix) -> void
    {
        DebugAssert(names.size() == out.size(), "Size mismatch!");
        static const std::string_view ApiDependentDir { Shader::GetShaderSubdir() };
        static constexpr std::string_view DirName { "GUI" };
        for (std::size_t i { }; i < names.size(); ++i)
        {
            const std::string fileName { Format("{}/{}/{}/{}_{}.bin", Shader::ShaderSearchDirectory, ApiDependentDir, DirName, names[i], suffix) };
            const bgfx::ShaderHandle handle { LoadCompiledShader(fileName.c_str()) };
            out[i] = handle;
        }
    }
    ...
    {
        Info("Vertex shaders generation date: {}", ::VertexVersion);
        Info("Fragment shaders generation date: {}", ::FragmentVersion);

        LoadShaders<VertexShaderCount_>(::VertexShaders, this->VertexShaders_, "VS");
        LoadShaders<FragmentShaderCount_>(::FragmentShaders, this->FragmentShaders_, "FS");
        ...
        for (std::size_t i { }; i < FragmentShaderCount_; ++i)
        {
            ...
            Assert(bgfx::isValid(this->Programs_[i]), "Failed to create program! VS: {}, FS: {}", ::VertexShaders[vsIdx], ::FragmentShaders[i]);
        }
    }
    ...
    ) -> Noesis::Ptr<Noesis::Texture>
    {
        Assert(numLevels == 1, "Invalid levels for GUI texture: {}", numLevels);
        ...
        switch (format)
        {
            ...
            case Noesis::TextureFormat::R8:
                ...
                break;
            default:
                Panic(Format("Invalid texture format for GUI texture: {}. Allowed are B8, RGBA8", static_cast<std::uint8_t>(format)));
        }
        return *new Texture
        {
            ...
            (
                ...
                data ? bgfx::copy(*data, width * height * stride) : nullptr
            ),
    ...
    {
        Assert(level == 0, "Invalid levels for GUI texture: {}", level);
    ...
    auto RenderDevice::DrawBatch(const Noesis::Batch& batch) -> void
    {
        ...
        Assert(vsValid, "Vertex count too large for transient buffer: {}", batch.numVertices);
        ...
        Assert(fsValid, "Index count too large for transient buffer: {}", batch.numIndices);
        ...
        if (batch.renderState.f.stencilMode)
        {
            ...
            {
                default: case Noesis::StencilMode::Disabled:    break;
                case Noesis::StencilMode::Equal_Keep:           stencil |= BGFX_STENCIL_OP_FAIL_S_KEEP | BGFX_STENCIL_OP_FAIL_Z_KEEP | BGFX_STENCIL_OP_PASS_Z_KEEP; break;
                case Noesis::StencilMode::Equal_Incr:           stencil |= BGFX_STENCIL_OP_FAIL_S_KEEP | BGFX_STENCIL_OP_FAIL_Z_KEEP | BGFX_STENCIL_OP_PASS_Z_INCR; break;
                case Noesis::StencilMode::Equal_Decr:           stencil |= BGFX_STENCIL_OP_FAIL_S_KEEP | BGFX_STENCIL_OP_FAIL_Z_KEEP | BGFX_STENCIL_OP_PASS_Z_DECR; break;
            }
        }
        if (batch.stencilRef)
        {
        ...
        DebugAssert(shaderIndex < FragmentShaderCount_, "Invalid shader index! Must be between 0 and {}", FragmentShaderCount_);
    ...
    auto RenderDevice::SetTextures(const Noesis::Batch& batch) noexcept -> void
    {
        auto set
        {
            [i = -1, this](Uniform& sampler, Noesis::Texture* texture, const Noesis::SamplerState& samplerState) mutable noexcept
            {
                ++i;
                if (texture)
                {
    ...
    auto RenderDevice::SetUniforms(const Noesis::Batch& batch) noexcept -> void
    {
        constexpr auto set
        {
            [](Uniform& target, const Noesis::UniformData& data) noexcept
            {
                if (data.values)
                {
